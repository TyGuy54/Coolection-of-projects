...
const unsigned int START_MEMORY_ADDRESS = 0x200;
...
void Chip8::loadRom(const char* filename){
    // opens a file as a binary stream
    ifstream romFile(filename, ios::binary | ios::ate); // the | is bitwise for OR

    if (romFile.is_open()){
        streampos fileSize = romFile.tellg();

        // The buffer hold the fileSize contents
        char* buffer = new char[fileSize]; // dynamicly allocateing memory to the buffer
        ...
        for (long i = 0; i < fileSize; i++){
            memory[START_MEMORY_ADDRESS + i] = buffer[i];
        }
...
// this one will take some time and research
void Chip8::Cycle(){
	// Fetch
...
void Chip8::OP_00E0(){
    // set the enire video buffer to 0
...
void Chip8::OP_2nnn(){
    // the subroutine is called, we want to return eventually, 
    // so we put the current PC onto the top of the stack
    uint16_t address = opcode & 0x0FFFu;
...
void Chip8::OP_3xkk(){
    // we increment by two to skip the next instruction
...
void Chip8::OP_8xy4(){
    ...
    // if the values of Vx and Vy are added togeahter and the result is greater than
    // 8 bits (sum > 225U) than Vf is set to 1 ortherwise 0
    if (sum > 225U) {
        registers[0xF] = 1;
    }
...
void Chip8::OP_8xy6(){
    ...
    // save east-significant bit in VF
    registers[0xF] = (registers[Vx] & 0x1u);

    registers[Vx] >>=1; // bitwise SHR "Shift Right" (>>)
}
